...
create table if not exists generation_outputs(id uuid primary key default gen_random_uuid(),generation_id uuid references generations(id) on delete cascade,image_url text,approved boolean default false,created_at timestamptz default now());
